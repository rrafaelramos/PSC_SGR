 ...
 * @author DONO 
 */
public abstract class PedidoFilhoDAO<T extends Pedido> extends EntidadeDAO<T> implements Repositorio<T>{

    protected PedidoDAO<T> pedidoRepositorio=new PedidoDAO();

    public PedidoFilhoDAO() throws ClassNotFoundException, SQLException {
        super();
    }
    ...
    public boolean Salvar(T filtro) throws ViolacaoRegraNegocioException { 
        ...
        if(pedidoRepositorio.Salvar(filtro)){
            try {
                PreparedStatement sql = null;

                HashMap<String,String> filhoPedido=new HashMap<>();
                filhoPedido.put("pedido", Integer.toString(filtro.getId()));
                
                if(Buscar(filhoPedido).isEmpty()){
